package org.saikumo.vams.security.filter;

import cn.hutool.core.util.StrUtil;
import lombok.extern.slf4j.Slf4j;
import org.saikumo.vams.entity.User;
import org.saikumo.vams.properties.JwtProperties;
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		if (StrUtil.isNotEmpty(authToken) && authToken.startsWith(jwtProperties.getTokenPrefix())) {
			...
			if (StrUtil.isNotEmpty(loginAccount) && SecurityContextHolder.getContext().getAuthentication() == null) {
				// 查询用户
				User userDetails = userRepository.findUserByUsername(loginAccount);
